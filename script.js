...
// Coading Challenge #4
/* Steve wants to build a very simple tip calculator for whenever he goes eating in a restutant. In his country, it's usual to tip 15% if the bill value is between 50 and 300 If the value is different, the tip is 20%.

1) Your task is to calculatae the tip depeding on the bill value. Create a variable called 'tip' for this. It's not allowed to use an if/else statement (If it's easier for you, you can start with an if/else statement, and then try to convert it to a ternary operator! ) 

2) Print a string to the cosole containing the bill value, the tip, and the final value (bill + tip). 
...
const total = (Number(tipValue) + Number(billValue));

console.log(`The bill was ${billValue} ,the tip was ${tipValue} , and the total value is ${total}`);
